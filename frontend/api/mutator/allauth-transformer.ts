import type { OpenAPIObject, PathItemObject, OperationObject, ParameterObject, ReferenceObject } from 'openapi3-ts/oas30';
 ...
 */
export default function allauthTransformer(schema: any): any {
  ...
  for (const [pathKey, pathObj] of Object.entries(schema.paths || {})) {
    ...
    const newPathObj: any = { ...(pathObj as object) };

    // Determine if this path has multiple HTTP methods
    const httpMethods = ['get', 'post', 'put', 'delete', 'patch'];
    const methodsInPath = Object.keys(newPathObj).filter(k => httpMethods.includes(k.toLowerCase()));
    const hasMultipleMethods = methodsInPath.length > 1;

    // Build the base Operation ID from the URL path
    const pathCleaned = pathKey.replace('/_allauth/{client}/v1/', '').replace('/_allauth/browser/v1/', '');
    ...
    for (const [method, methodObj] of Object.entries(newPathObj)) {
      if (typeof methodObj === 'object' && methodObj !== null) {
        const typedMethodObj = methodObj as any;
        ...
        if (typedMethodObj.parameters) {
          ...
          typedMethodObj.parameters = typedMethodObj.parameters.filter(
            (p: any) => p.$ref !== '#/components/parameters/Client'
          );
